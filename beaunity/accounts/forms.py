...
class AppUserCreationForm(UserCreationForm):
    ...
    email = forms.EmailField(
        widget=forms.EmailInput(attrs={"class": CLASS}),
    )
    password1 = forms.CharField(
        ...
        widget=forms.PasswordInput(attrs={"class": CLASS}),
    )
...
class AppUserLoginForm(AuthenticationForm):
    username = forms.CharField(
        label="Username or email", widget=forms.TextInput(attrs={"class": CLASS})
    )
    password = forms.CharField(
        label="Password", widget=forms.PasswordInput(attrs={"class": CLASS})
    )
...
class AppUserEditForm(forms.ModelForm):
    ...
    email = forms.EmailField(
        label="Email:", widget=forms.EmailInput(attrs={"class": CLASS})
    )
...
class ProfileBaseForm(forms.ModelForm):
    ...
    profile_pic = CloudinaryFileField(
        ...
        widget=ClearableFileInput(attrs={"class": CLASS}),
    )
    ...
    date_of_birth = forms.DateField(
        ...
        widget=forms.DateInput(
            attrs={"class": CLASS, "type": "date"}, format="%Y-%m-%d"
        ),
        input_formats=["%Y-%m-%d"],
    )
    bio = forms.CharField(
