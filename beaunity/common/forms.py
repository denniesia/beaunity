...
class ContactForm(forms.Form):
    ...
    email = forms.EmailField(
        label="Your Email:", widget=forms.EmailInput(attrs={"class": CLASS})
    )
    subject = forms.CharField(
        max_length=100, label="Subject:", widget=forms.TextInput(attrs={"class": CLASS})
    )
    content = forms.CharField(
        max_length=1000, label="Content:", widget=forms.Textarea(attrs={"class": CLASS})
    )
...
class ActivityBaseForm(forms.ModelForm):
    ...
    is_online = forms.BooleanField(
        label="Online event:", required=False, widget=forms.CheckboxInput()
    )
