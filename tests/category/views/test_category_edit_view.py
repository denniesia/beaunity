...
class TestsCategoryEditView(TestCase):
    def setUp(self):
        ...
        self.staff = UserModel.objects.create_user(
            username="staffuser", email="staffuser@test.bg", password="dfgm!2"
        )
        ...
        self.url = reverse(
            "category-edit", kwargs={"category_slug": self.category.slug}
        )
    ...
    def test_get_edit_category_page_with_staff_user(self):
        self.client.login(username="staffuser", password="dfgm!2")
        response = self.client.get(self.url)
